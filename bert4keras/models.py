...
class Transformer(object):
    ...
    def load_checkpoint(self, checkpoint):
        ...
        return tf.train.load_checkpoint(checkpoint)
    ...
    def load_weights_from_checkpoint(self, checkpoint, mapping=None):
        ...
        weight_value_pairs = []
        checkpoint_reader = self.load_checkpoint(checkpoint)
        for layer, variables in tqdm(mapping.items()):
            layer = self.layers[layer]
            weights, values = [], []

            for w, v in zip(layer.trainable_weights, variables):  # 允许跳过不存在的权重
                try:
                    values.append(checkpoint_reader.get_tensor(v))
                    weights.append(w)
            ...
            for i, (w, v) in tqdm(enumerate(zip(weights, values))):
                if v is not None:
        ...
        gc.collect()
        while len(weight_value_pairs) > 0:
            w, v = weight_value_pairs.pop()
            K.set_value(w, v)
            gc.collect()
