...
class Transformer(object):
    ...
    def call(self, inputs):
        ...
        # return outputs
        outputs_source = self.apply_embeddings(inputs[:1])
        # Main
        for i in range(self.num_hidden_layers):
            outputs_source = self.apply_main_layers(outputs_source, i)
        # Final
        outputs_source = self.apply_final_layers(outputs_source)

        outputs_target = self.apply_embeddings(inputs[1:])
        # Main
        for i in range(self.num_hidden_layers):
            outputs_target = self.apply_main_layers(outputs_target, i)
        # Final
        outputs_target = self.apply_final_layers(outputs_target)
        return [outputs_source, outputs_target]
...
class XLM_ROBERTA(Transformer):
    """构建BERT模型
    """
    def __init__(
        self,
        max_position,  # 序列最大长度
        segment_vocab_size=1,  # segment总数目
        with_pool=False,  # 是否包含Pool部分
        with_nsp=False,  # 是否包含NSP部分
        with_mlm=False,  # 是否包含MLM部分
        hierarchical_position=None,  # 是否层次分解位置编码
        ...
        **kwargs  # 其余参数
        ...
        super(XLM_ROBERTA, self).__init__(**kwargs)
        self.max_position = max_position
        self.segment_vocab_size = segment_vocab_size
        self.with_pool = with_pool
        self.with_nsp = with_nsp
    ...
    def get_inputs(self):
        ...
        x_in_target = Input(shape=(self.sequence_length,), name='Input-Token_target')
        inputs = [x_in, x_in_target]
    ...
    def load_variable(self, checkpoint, name):
        """加载单个变量的函数
        """
        variable = super(XLM_ROBERTA, self).load_variable(checkpoint, name)
        if name in [
            'bert/embeddings/word_embeddings',
            'cls/predictions/output_bias',
        ]:
    ...
    def create_variable(self, name, value, dtype=None):
        ...
        if name == 'cls/seq_relationship/output_weights':
            value = value.T
        return super(XLM_ROBERTA, self).create_variable(name, value, dtype)
    ...
    def variable_mapping(self):
        ...
        mapping = {
            ...
            'NSP-Proba': [
                'bert/cls/seq_relationship/output_weights',
                'bert/cls/seq_relationship/output_bias',
            ],
            'MLM-Dense': [
                'bert/cls/predictions/transform/dense/kernel',
                'bert/cls/predictions/transform/dense/bias',
            ],
            'MLM-Norm': [
                'bert/cls/predictions/transform/LayerNorm/beta',
                'bert/cls/predictions/transform/LayerNorm/gamma',
            ],
            'MLM-Bias': ['bert/cls/predictions/output_bias'],
        }
