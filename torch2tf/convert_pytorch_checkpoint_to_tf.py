...
from transformers.models.auto import AutoModelForMaskedLM as BertModel
...
def convert_pytorch_checkpoint_to_tf(model:BertModel, ckpt_dir:str, model_name:str, config:dict):
    ...
    var_map = (
        ...
        ('lm_head.decoder.weight', 'cls/predictions/output_weight'),
    ...
    with tf.Session() as session:
        for var_name in state_dict:
            if var_name == 'lm_head.decoder.bias': continue
            if var_name =='lm_head.decoder.weight': continue
            ...
            if 'cls/predictions/output_bias' in tf_name:
                add_emb_shape = config['vocab_size'] - torch_tensor.shape[0]
                embedding_table = tf.get_variable(name='additional_bias',
                                                  shape=[add_emb_shape],
